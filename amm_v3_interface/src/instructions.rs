...
impl AmmV3ProgramIx {
    pub fn deserialize(buf: &[u8]) -> std::io::Result<Self> {
        ...
        match maybe_discm {
            ...
            UPDATE_OPERATION_ACCOUNT_IX_DISCM => {
                Ok(
                    Self::UpdateOperationAccount(
                        UpdateOperationAccountIxArgs::deserialize(&mut reader)?,
                    ),
                )
            }
            TRANSFER_REWARD_OWNER_IX_DISCM => {
                Ok(
                    Self::TransferRewardOwner(
                        TransferRewardOwnerIxArgs::deserialize(&mut reader)?,
                    ),
                )
            }
            ...
            COLLECT_REMAINING_REWARDS_IX_DISCM => {
                Ok(
                    Self::CollectRemainingRewards(
                        CollectRemainingRewardsIxArgs::deserialize(&mut reader)?,
                    ),
                )
            }
            UPDATE_REWARD_INFOS_IX_DISCM => Ok(Self::UpdateRewardInfos),
            SET_REWARD_PARAMS_IX_DISCM => {
                Ok(
                    Self::SetRewardParams(
                        SetRewardParamsIxArgs::deserialize(&mut reader)?,
                    ),
                )
            }
            ...
            OPEN_POSITION_V2_IX_DISCM => {
                Ok(Self::OpenPositionV2(OpenPositionV2IxArgs::deserialize(&mut reader)?))
            }
            ...
            SWAP_ROUTER_BASE_IN_IX_DISCM => {
                Ok(
                    Self::SwapRouterBaseIn(
                        SwapRouterBaseInIxArgs::deserialize(&mut reader)?,
                    ),
                )
            }
            _ => {
...
pub fn create_pool_verify_account_keys(
    ...
    for (actual, expected) in [
        ...
        (*accounts.token_mint0.key, keys.token_mint0),
        (*accounts.token_mint1.key, keys.token_mint1),
        ...
        (*accounts.token_program0.key, keys.token_program0),
        (*accounts.token_program1.key, keys.token_program1),
        (*accounts.system_program.key, keys.system_program),
...
pub const CREATE_OPERATION_ACCOUNT_IX_DISCM: [u8; 8] = [
    63,
    87,
    148,
    33,
...
pub fn create_operation_account_ix(
    ...
    create_operation_account_ix_with_program_id(crate::ID, keys)
}
...
pub fn create_operation_account_invoke(
    ...
    create_operation_account_invoke_with_program_id(crate::ID, accounts)
}
...
pub fn create_operation_account_invoke_signed(
    ...
    create_operation_account_invoke_signed_with_program_id(crate::ID, accounts, seeds)
}
...
pub const UPDATE_OPERATION_ACCOUNT_IX_DISCM: [u8; 8] = [
    127,
    70,
    119,
    40,
...
pub const TRANSFER_REWARD_OWNER_IX_DISCM: [u8; 8] = [7, 22, 12, 83, 242, 43, 48, 121];
#[derive(BorshDeserialize, BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
...
pub const COLLECT_REMAINING_REWARDS_IX_DISCM: [u8; 8] = [
    18,
    237,
    166,
    197,
...
pub fn collect_remaining_rewards_verify_writable_privileges<'me, 'info>(
    ...
    for should_be_writable in [accounts.funder_token_account, accounts.pool_state] {
        if !should_be_writable.is_writable {
            return Err((should_be_writable, ProgramError::InvalidAccountData));
        }
    }
...
pub const UPDATE_REWARD_INFOS_IX_DISCM: [u8; 8] = [163, 172, 224, 52, 11, 154, 106, 223];
#[derive(Clone, Debug, PartialEq)]
...
pub fn update_reward_infos_ix(
    ...
    update_reward_infos_ix_with_program_id(crate::ID, keys)
}
...
pub fn update_reward_infos_invoke(
    ...
    update_reward_infos_invoke_with_program_id(crate::ID, accounts)
}
...
pub fn update_reward_infos_invoke_signed(
    ...
    update_reward_infos_invoke_signed_with_program_id(crate::ID, accounts, seeds)
}
...
pub const SET_REWARD_PARAMS_IX_DISCM: [u8; 8] = [112, 52, 167, 75, 32, 201, 211, 137];
#[derive(BorshDeserialize, BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
...
pub const OPEN_POSITION_V2_IX_DISCM: [u8; 8] = [77, 184, 74, 214, 112, 86, 241, 199];
#[derive(BorshDeserialize, BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
...
pub const SWAP_ROUTER_BASE_IN_IX_DISCM: [u8; 8] = [
    69,
    125,
    115,
    218,
